...
export function contractTemplate(c: CaseDocData): TDocumentDefinitions {
  ...
  return {
    ...
    content: [
      ...
      { text: `Toshkent sh., ${line?.lineDate ? dateToUzbekWords(line.lineDate) : '—'}`, alignment: 'center', margin: [0, 0, 0, 14] },
      ...
      { table: { widths: [180, '*'], body: [
        ...
        kv('Yillik foiz stavkasi', line?.interestRate != null ? `${Math.round(Number(line.interestRate) * 100)}%` : '—'),
        kv("Muddati o'tgan uchun", line?.penaltyRate != null ? `${Math.round(Number(line.penaltyRate) * 100)}%` : '—'),
        kv("To'lov turi", tr?.scheduleType === 'DIFFERENTIATED' ? 'differensial' : (tr?.scheduleType === 'ANNUITY' ? 'annuitet' : '—')),
      ] } },
