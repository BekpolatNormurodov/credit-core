...
/**
 * Read numbered fields "N. value" out of OCR text. Lines with no leading number are treated as a
 * continuation of the previous field (addresses wrap across lines). Returns a map of number → value.
 */
export function numberedFields(text: string): Map<number, string> {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    } else if (current != null && current <= 7) {
      // Only wrap onto text fields (owner/address/issuer); numeric back fields never wrap.
...
function splitVin(raw: string): { bodyNo: string; chassis: string } {
  ...
  return { bodyNo: alnum(body), chassis: chassisPart ? squish(chassisPart) : '' };
}
...
export function extractTexFields(frontText: string, backText: string): TexScanResult {
  ...
  if (front.get(1)) f.stateNumber = alnum(front.get(1)!);
  ...
  if (back.get(14)) f.engineNo = alnum(back.get(14)!);
