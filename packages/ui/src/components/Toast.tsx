...
const toneMap: Record<ToastTone, { icon: React.ComponentType<{ className?: string }>; ring: string; chip: string }> = {
  success: { icon: Check, ring: 'border-l-success-500', chip: 'bg-success-50 text-success-600 dark:bg-success-600/15' },
  error: { icon: Warning, ring: 'border-l-danger-500', chip: 'bg-danger-50 text-danger-600 dark:bg-danger-600/15' },
  info: { icon: Info, ring: 'border-l-brand-500', chip: 'bg-brand-50 text-brand-700 dark:bg-brand-600/15' },
};
            ...
            {toasts.map((t) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    'pointer-events-auto flex items-start gap-3 rounded-xl border border-l-4 border-hairline bg-surface p-3.5 shadow-card',
                ...
                >
                  ...
                  <button onClick={() => remove(t.id)} className="-mr-1 shrink-0 rounded-md p-0.5 text-slate-400 transition hover:text-ink dark:hover:text-white">
                    <X className="h-4 w-4" />
                  </button>
                </motion.div>
              );
